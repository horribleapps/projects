...
def capture_and_detect_circles():
    ...
    # Variable to define when the while loop ends
    runvar=True
    looprange=100
    ...
    while runvar:
        for frameidx in range(looprange):
            ...
            if circles is not None:
                ...
                for (x, y, r) in circles:
                    #circounter = circounter+1 if ((r>25)&(r<50)) else circounter
                    circounter = circounter+1 if ((r>14)&(r<21)) else circounter
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                runvar=False
                break
        ratio=circounter/looprange
        if ratio>0.5:
            print("Circle found! Move away!")
            distance=distance*(-1)
        #This is used to move the cnc machine a certain distance
        mc.move_obj(distance,speed,axis)
        circounter=0
